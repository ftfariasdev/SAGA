...
async function syncProfessoresTurmas() {
  try {
    ...
    for (const materia of materiasComProfessor) {
      ...
      console.log(
        `Verificando vínculos do professor ${professor.id_professor} com ${turmas.length} turmas do curso ${materia.curso.nome}`
      );
      ...
      for (const turma of turmas) {
        ...
        if (!vinculoExistente) {
          ...
          vinculosCriados++;
          console.log(
            `Vínculo criado: Professor ${professor.id_professor} - Turma ${turma.id_turma} (${turma.nome})`
          );
        }
      }
    }
