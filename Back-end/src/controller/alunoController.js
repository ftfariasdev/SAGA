...
export class AlunoController {
  async listInfoCurso(req, res) {
    try {
      ...
      const materias = (aluno.turma.curso.materias || []).map((materia) => ({
        ...
        professor:
          materia.professor && materia.professor.user ? materia.professor.user.nome : 'Não definido'
      }));
      ...
      return res
        .status(500)
        .json({ erro: 'Erro ao listar informações do curso', detalhes: error.message });
    }
  }
  ...
  async listModuloInfo(req, res) {
    try {
      ...
      const materiasModulo = aluno.turma.curso.materias.filter((materia) =>
        materia.codigo.toString().startsWith(modulo)
      );
      ...
      const materiasInfo = await Promise.all(
        materiasModulo.map(async (materia) => {
      ...
      return res
        .status(500)
        .json({ erro: 'Erro ao listar informações do módulo', detalhes: error.message });
    }
  }
  ...
  async getFrequenciaByData(req, res) {
    try {
      ...
      if (!data) {
        return res
          .status(400)
          .json({ erro: 'Data não fornecida. Use o parâmetro ?data=YYYY-MM-DD' });
      }
      ...
      try {
        ...
        return res
          .status(400)
          .json({ erro: 'Formato de data inválido. Use YYYY-MM-DD', detalhes: err.message });
      }
      ...
      if (!presenca) {
        return res
          .status(404)
          .json({ erro: 'Presença do aluno não encontrada na chamada desta data.' });
      }
      ...
      return res
        .status(500)
        .json({ erro: 'Erro ao buscar frequência por data', detalhes: error.message });
    }
  }
  ...
  async getPresencasByDia(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ erro: 'Erro ao buscar presenças do dia', detalhes: error.message });
    }
  }
}
