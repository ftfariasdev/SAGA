...
export class ProfController {
  async listarTurmasProf(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ erro: 'Erro ao buscar turmas do professor', detalhes: error.message });
    }
  }
  ...
  async listarAlunosTurma(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ erro: 'Erro ao buscar alunos da turma', detalhes: error.message });
    }
  }
  async materiasProf(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ erro: 'Erro ao buscar matérias do professor', detalhes: error.message });
    }
  }
  ...
  async realizarChamada(req, res) {
    try {
      ...
      if (!id_turma || !data || !Array.isArray(presencas)) {
        return res
          .status(400)
          .json({ erro: 'Dados obrigatórios: id_turma, data e presencas (array).' });
      }
      ...
      for (const presenca of presencas) {
        ...
        if (typeof id_aluno !== 'string' || typeof presente !== 'boolean') {
          return res
            .status(400)
            .json({ erro: 'Cada presença deve ter id_aluno (string) e presente (boolean).' });
        }
  ...
  async listarChamada(req, res) {
    try {
      ...
      const chamadas = await prisma.chamada.findMany({
        ...
        orderBy: [{ data: 'desc' }, { id_aluno: 'asc' }]
      });
    ...
    try {
      ...
      if (!id_turma || !id_materia || !tipo_avaliacao || !bimestre || !Array.isArray(notas)) {
        return res.status(400).json({
          erro: 'Dados obrigatórios: id_turma, id_materia, tipo_avaliacao, bimestre e notas (array).'
        });
      }
      ...
      for (const notaAluno of notas) {
        ...
        if (typeof id_aluno !== 'string' || typeof valor !== 'number') {
          return res
            .status(400)
            .json({ erro: 'Cada nota deve ter id_aluno (string) e valor (number).' });
        }
      ...
      return res
        .status(200)
        .json({ mensagem: 'Notas lançadas/atualizadas com sucesso!', resultados });
    } catch (error) {
  ...
  async buscarChamadaPorData(req, res) {
    try {
      ...
      return res
        .status(500)
        .json({ erro: 'Erro ao buscar chamada por data', detalhes: error.message });
    }
  }
}
