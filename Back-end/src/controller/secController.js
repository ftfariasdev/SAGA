...
export class SecController {
  ...
  async editarCurso(req, res) {
    ...
    const curso = await prisma.curso.update({
      where: { id_curso },
      data: { nome, periodo, descricao, ch_total, freq_min }
    });
    if (!curso) {
  ...
  async cadProfessor(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Erro ao cadastrar professor', detalhes: error.message });
    }
  ...
  async cadSecretaria(req, res) {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ error: 'Erro ao cadastrar secretaria', detalhes: error.message });
    }
  }
  ...
  async excluirUsuario(req, res) {
    ...
    try {
      ...
      } else if (usuarioExistente.tipo === 2) {
        // Professor
      ...
      } else if (usuarioExistente.tipo === 1) {
        // Secretaria
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        if (error.code === 'P2025') {
          ...
          return res.status(500).json({
            message:
              'Erro ao excluir usuário: existem registros relacionados que precisam ser removidos primeiro.',
