...
document.addEventListener('DOMContentLoaded', function () {
  ...
  loginForm.addEventListener('submit', async function (event) {
    ...
    try {
      ...
      showErrorMessage(
        error.message || 'Erro ao conectar ao servidor. Verifique se o backend está rodando.'
      );
      resetLoginButton();
    }
  });
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (errorMessage.includes('Cadastro não existe')) {
          ...
          alert(
            'Cadastro não existe. Entre em contato com a secretaria para realizar seu cadastro.'
          );
