    ...
    try {
      if (tipo === 3) {
        ...
        const professorResponse = await fetch(
          `http://localhost:8081/sec/consultarProfessor/${id_user}`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        const secretariaResponse = await fetch(
          `http://localhost:8081/sec/consultarSecretaria/${id_user}`,
          {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
    ...
    if (usuarioAtual) {
      if (usuarioAtual.tipo === 3) {
        ...
        if (turmasSelect) {
          ...
          const turmasSelecionadas = Array.from(turmasSelect.selectedOptions).map(
            (option) => option.value
          );
