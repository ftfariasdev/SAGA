document.addEventListener('DOMContentLoaded', () => {
  ...
  async function excluirTurma(id_turma) {
    if (
      !confirm(
        'Tem certeza que deseja excluir esta turma? Todos os alunos e professores vinculados serão desassociados.'
      )
    ) {
      return;
    }
  ...
  function exibirTurmas(turmas) {
    ...
    turmas.forEach((turma) => {
      // Contar professores vinculados à turma através da relação professoresRelation
      ...
      if (numProfessores > 0) {
        const nomesProfessores = turma.professoresRelation
          .map((rel) => {
