...
async function carregarCursos() {
  ...
  try {
    ...
    mostrarModal(
      'Não foi possível carregar a lista de cursos. Por favor, tente novamente mais tarde.'
    );
  }
}
...
async function cadastrarTurma(event) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        responseData.erro || responseData.error || responseData.message || 'Erro ao cadastrar turma'
      );
    }
